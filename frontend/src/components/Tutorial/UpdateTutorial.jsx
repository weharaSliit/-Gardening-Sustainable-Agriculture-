...
export default function UpdateTutorial() {
  ...
  useEffect(() => {
    const fetchTutorial = async () => {
      try {
        ...
        setFormData({
          ...tutorialData,
          categories: tutorialData.categories ? tutorialData.categories.join(', ') : '',
          tutorialImage: null, // We don't want to upload the image again unless changed
          mediaFiles: [] // Same for media files
        });
        
        // Set preview image if exists
        if (tutorialData.tutorialImage) {
          setPreviewImage(`http://localhost:8080/api/v1/tutorial/viewtutorial/image/${tutorialData.tutorialImage}`);
        }
  ...
  const addStep = e => {
    ...
    setFormData(f => ({
      ...
      ].sort((a, b) => a.stepNumber - b.stepNumber) // Sort by step number
    }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare tutorial details as JSON
      ...
      updateFormData.append('tutorialDetails', JSON.stringify(tutorialDetails));
      
      // Add image if present
      if (formData.tutorialImage) {
        updateFormData.append('file', formData.tutorialImage);
      }
      
      // Add media files if present
      if (formData.mediaFiles.length > 0) {
        formData.mediaFiles.forEach(file => {
          updateFormData.append('mediaFiles', file);
        });
      }
      
      // Send update request
      await axiosClient.put(`/tutorial/updatetutorial/${id}`, updateFormData, {
